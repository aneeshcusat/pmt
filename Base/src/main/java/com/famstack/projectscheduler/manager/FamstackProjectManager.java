...
public class FamstackProjectManager extends BaseFamstackManager
{
		            			...
		            			if (projectType.equalsIgnoreCase("BILLABLE")) {
		            			   ...
		            			   if ("LEAVE".equalsIgnoreCase(taskNameOrId) || "MEETING".equalsIgnoreCase(taskNameOrId)) {
		            				...
		            				famstackUserActivityManager.createCompletedUserActivityItem(taskDetails.getAssignee(), weekStartTime, 0, taskName,
		            						dayTaskTime, UserTaskType.valueOf(userTaskType), taskActCategory, ProjectType.NON_BILLABLE, taskCommentsMap.get(taskNameOrId), null, null, null, null, null, null, null, null);
		            			}
            				}
	...
	public List<POEstimateProjectTaskActivityDetails> getAllProjectPOEstimateDuration(
    ...
    private void mapPOEstimateProjectsList(
			List<POEstimateProjectTaskActivityDetails> projectDetailsList,
	...
	public void sendAutoReportEmail(List<String> toList, List<String> ccList, List<String> excludeMailList,
		...
		if (reportType == ReportType.USER_SITE_ACTIVITY) {
			...
			if (notifyDefaulters){
				List<String> emailList = !ccList.isEmpty() ?  ccList : toList;
				notifyUserUnderOverUtilized(utilizationDetails, new HashSet<>(emailList), dateRange, userGroupDetails.getName(), subject, NotificationType.USER_UTILIZATION_REPORT_DEFAULTER);
			}
			}
			...
			startDate = DateUtils.getFirstDayOfPrevioushWeek(DateUtils.getNextPreviousDate(DateTimePeriod.DAY, new Date(), -1 * 3));
			endDate = DateUtils.getLastDayOfPreviousWeek(DateUtils.getNextPreviousDate(DateTimePeriod.DAY, new Date(), -1 * 3));
			...
			if (projectTaskActivityDetails != null && !projectTaskActivityDetails.isEmpty()) {
				...
				famstackNotificationServiceManager.notifyAll(
						NotificationType.WEEKLY_PO_ESTIMATION_REPORT,
						notificationDataMap, null);
			}
		}
	}
	...
	if (startDate != null && endDate != null) {
		...
		if (projectTaskAssigneeDataList != null && !projectTaskAssigneeDataList.isEmpty()) {
			for (ProjectTaskActivityDetails projectDetails : projectTaskAssigneeDataList) {
				...
				String skillOrCategory = projectDetails.getProjectCategory() == null ? "Other" : projectDetails.getProjectCategory();
				
				if(projectDetails.getProjectCategory() == null) {
					System.out.println("ProjectCategory NULL : " + projectDetails);
				}
				
				if (reportType == ReportType.UTILIZATION_BY_EMPLOYEE_BY_PROJECT_CATEGORY && "Holiday".equalsIgnoreCase(skillOrCategory) || "Leave".equalsIgnoreCase(skillOrCategory)){
					continue;
				}
				...
				Map<Integer, Map<String, Map<String, Integer>>> userIdSkillHoursMap = monthYearUserIdSkillHoursMap.get(monthYear);
				...
				if (reportType != ReportType.UTILIZATION_BY_EMPLOYEE_BY_PROJECT_CATEGORY) {
					...
					skillOrCategory = employeeDetails !=null && StringUtils.isNotBlank(employeeDetails.getSkills()) ? employeeDetails.getSkills()  : "Other";
				}
		...
		for(String monthYear : monthYearUserIdSkillHoursMap.keySet()) {
			...
			for(Integer userId : monthYearUserIdSkillHoursMap.get(monthYear).keySet()) {
				...
				for(String skillOrCategory : monthYearUserIdSkillHoursMap.get(monthYear).get(userId).keySet()) {
					...
					Map<String, Integer> hoursMap = monthYearUserIdSkillHoursMap.get(monthYear).get(userId).get(skillOrCategory);
					
					for(String type : hoursMap.keySet()) {
						if ("billable".equalsIgnoreCase(type)) {
							utilizationByUserSkillOrCategory
				...
				if (reportType == ReportType.UTILIZATION_BY_SKILLS) {
					Collections.sort(skillList, new Comparator<UtilizationByUserSkillOrCategory>() {
						@Override
						public int compare(
								UtilizationByUserSkillOrCategory utilizationBySkill1,
								UtilizationByUserSkillOrCategory utilizationBySkill2) {
								return utilizationBySkill1.getSkillOrCategory().compareTo(
										utilizationBySkill2.getSkillOrCategory());
						}
					});
				}
	...
	if (reportType != ReportType.UTILIZATION_BY_SKILLS) {
		Collections.sort(utilizationBySkillList, new Comparator<UtilizationByUserSkillOrCategory>() {
			...
			public int compare(
					UtilizationByUserSkillOrCategory utilizationBySkill1,
					UtilizationByUserSkillOrCategory utilizationBySkill2) {
				return utilizationBySkill1.getEmployeeName().compareTo(
						utilizationBySkill2.getEmployeeName());
			}
		});
	}
	...
	private List<POEstimateProjectTaskActivityDetails> getAutoReportPOEstimationDataForEmail(
		...
		if (startDate != null && endDate != null) {
			projectTaskAssigneeDataList.addAll(getAllProjectPOEstimateDuration(
	...
	private void notifyUserUnderOverUtilized(
			...
			HashSet<String> ccEmailList, String dateRange, String teamName, String subject, NotificationType notificationType) {
		if (utilizationDetails != null) {
			Set<String> emailIds = new HashSet<>();
