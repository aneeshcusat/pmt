...
public class FamstackDesktopNotificationService extends FamstackBaseNotificationService {
	...
	private final Map<Integer, Map<Integer, LimitedQueue<GroupMessageDetails>>> userMessageNotificationMap = new HashMap<>();
			...
			if (subscriberIds.length > 0) {
				for (Integer subscriberId : subscriberIds) {
					if (subscriberId != currentUserId) {
						Map<Integer, LimitedQueue<GroupMessageDetails>> userMessageMap = userMessageNotificationMap
								.get(subscriberId);
